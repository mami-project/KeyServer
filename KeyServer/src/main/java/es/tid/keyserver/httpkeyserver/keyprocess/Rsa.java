 ...
 */
package es.tid.keyserver.httpkeyserver.keyprocess;
...
public class Rsa {
    ...
    static public String calcDecodedOutput(String data, PrivateKey key) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException{
        ...
        // Decoding incomming data using PrivateKey and gets the PremasterSecret decoded.
        byte[] decodifiedData = decrypt(codifiedData, key);
        // Check if something goes wrong when decrypt.
        if (decodifiedData == null){
            return null;
        }
        // Enconde the PremasterSecret using base64.
        return Base64.getEncoder().encodeToString(decodifiedData);
    }
    
    /**
     * This method is used to decrypt using RSA an array of bytes using the private key object.
